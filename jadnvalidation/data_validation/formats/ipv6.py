...
class Ipv6:
    ...
    def __init__(self, data: any = None):
        ...
        if isinstance(self.data, str):
            self.string_variable = data
        elif isinstance(self.data, bytes):   
            self.string_variable = self.data.decode('utf-8')
        else: self.errors.append(f"Data does not match ipv6: {self.data_str}")
    ...
    def validate(self):
        ...
        """
        try:
            assert ipaddress.IPv6Address(self.string_variable)
            ...
            raise ValueError(f"Invalid ipv6 format: {self.string_variable}")